...
PFN_RxCallback Rx1_Callback;
PFN_RxCallback Rx5_Callback;
...
int UART_Enable(UART_ID_t uartId, uint32_t baud)
{
	...
	uint32_t busClockFreq = PLL_BusClockFreq;
	...
	switch (uartId) {
		...
		case UART1:
			// Rx=PB0, Tx=PB1
			GPIO_EnableAltDigital(PORTB, PIN0 | PIN1, 0x1);
			break;
...
//	priority = (priority << 1) & 0x0E;
	...
	switch (uartId) {
		...
		case UART1:
			...
			NVIC_EnableIRQ(INT_UART1 - 16, priority);
...
//			NVIC_EN1_R |= 0x20000000; // bit 29
			...
			NVIC_EnableIRQ(INT_UART5 - 16, priority);
			Rx5_Callback = callback;
			break;
...
static void RxHandler(volatile UARTRegs_t* uart, PFN_RxCallback callback)
{
...
// UART1 Rx interrupt handler.  Must be configured in the IRQ vector table.
void UART_Rx1Handler(void)
{
	volatile UARTRegs_t* uart = (volatile UARTRegs_t*)UARTBaseAddress[UART1];
	RxHandler(uart, Rx1_Callback);
}


// UART5 Rx interrupt handler.  Must be configured in the IRQ vector table.
void UART_Rx5Handler(void)
{
	volatile UARTRegs_t* uart = (volatile UARTRegs_t*)UARTBaseAddress[UART5];
	RxHandler(uart, Rx5_Callback);
}