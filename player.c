...
static TrackParams_t trackParams_[MAX_TRACKS];
...
static void LoadSong(int index) 
{
	if (album_.header == NULL) {
		...
		uint8_t* address = (uint8_t*)album_.header + album_.songOffset[index];
		
		FileHeader_t* pSongFileHeader = (FileHeader_t *)address;
		if (pSongFileHeader->signature != FILE_SIGNATURE) {
			Display_ShowFileNotLoadedScreen();
		}
		else {
		
			song_.header = pSongFileHeader;
			address += sizeof(FileHeader_t);
			
			for (int i = 0; i < song_.header->numTracks; i++) {
				
				TrackHeader_t *pTrackHeader = (TrackHeader_t *)address;
				song_.tracks[i].notes = (NoteEvent_t*)((uint8_t*)song_.header + pTrackHeader->offset);
				
				address += sizeof(TrackHeader_t);
			}
...
{
	album_.header = NULL;
	album_.songOffset = NULL;
	playerState_.songIndex = -1;
	
	uint8_t* address = (uint8_t*)pConfig_->flashBaseAddress;
	
	AlbumHeader_t* pAlbumHeader = (AlbumHeader_t *)address;
	if (pAlbumHeader->signature != ALBUM_SIGNATURE) {
		Display_ShowFileNotLoadedScreen();
	}
	else if (pAlbumHeader->numSongs > 0) {
		
		album_.header = pAlbumHeader;
		album_.songOffset = (uint32_t*)(address + sizeof(AlbumHeader_t));
...
static void ButtonHandler(Ports_t port, uint32_t pinMap)
{
...
static void PlayNote(const NoteEvent_t* note, TrackParams_t* params)
{
	...
	if (!note || note->key == 0 || note->velocity == 0) {
		...
		*(params->led) = 0;
	...
	else if (note->key >= MIN_NOTE && note->key <= MAX_NOTE) {
		...
		if (channel == 9) {
			...
			uint16_t period = NotePwmPeriods[note->key];
...
void TrackTask(void *pvParameters)
{
	...
	for (;;) {
		...
		if (taken == pdPASS) {
			...
			const NoteEvent_t* notes = params->track->notes;
